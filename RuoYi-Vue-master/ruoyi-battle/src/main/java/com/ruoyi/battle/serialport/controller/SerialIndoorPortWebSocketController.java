 ...
 */
@Component
@ServerEndpoint("/socket/in")
public class SerialIndoorPortWebSocketController {

    private final Logger log = LoggerFactory.getLogger(getClass());

    private static final String heartbeatTag = "pong";
    private static Map<String, Session> sessionMap = new ConcurrentHashMap<>();
    private static SerialIndoorPortService serialIndoorPortService;

    @Autowired
    public void setSerialIndoorPortService(SerialIndoorPortService serialIndoorPortService) {
        SerialIndoorPortWebSocketController.serialIndoorPortService = serialIndoorPortService;
    }

    @OnOpen
    public void onOpen(Session session) throws IOException {
        sessionMap.put(session.getId(), session);
//        log.info("室内天线有新连接{}加入！当前在线人数为{}", session.getId(), sessionMap.size());
        if (null != serialIndoorPortService.getNowComStatus()) {
...
//        log.info("室内天线有一连接{}关闭！当前在线人数为{}", session.getId(), sessionMap.size());
    }

    @OnMessage
    public void onMessage(String message, Session session) throws IOException {
//        log.info("室内天线收到{}心跳信息：{}", session.getId(), message);
        if (null != serialIndoorPortService.getNowComStatus()) {
            session.getBasicRemote().sendText(HeartbeatResponse.HeartbeatResponseBuilder
                    .aHeartbeatResponse()
                    .code(HttpServletResponse.SC_OK)
    ...
    public void onError(Session session, Throwable error) {
        log.error("室内天线WebSocket通信发生异常", error);
    }
